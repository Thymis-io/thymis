...
Only an outbound HTTP(S) connection to `A_CUSTOM_DOMAIN.thymis.cloud` and `RANDOM.tc.thymis.cloud` is required, which will be upgraded to a WebSocket connection. They work seamlessly behind NAT without requiring any inbound ports.
