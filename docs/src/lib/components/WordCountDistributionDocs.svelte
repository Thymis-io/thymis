<script lang="ts">
	...
	let categoryStats = $derived.by(() => {
		...
		return Object.entries(categorizedWordCounts).map(([category, count]) => {
			...
			const displayName =
				category === 'veryShort' ? 'very short' : category === 'veryLong' ? 'very long' : category;
			return {
				category,
				count,
				percentage,
...
<div class="mb-6 rounded-lg border border-gray-200 bg-gray-50 p-4">
	...
	<div class="mb-4">
		...
		<div class="flex h-3 flex-row-reverse overflow-hidden rounded-full bg-gray-200">
			...
			<A href={randomPageForCategories[stat.category] || '#'}>
				<div class="flex items-center justify-between text-xs">
					<div class="flex min-w-0 flex-1 items-center gap-2">
						...
						></div>
						<span class="truncate text-gray-600 capitalize">{stat.displayName}</span>
						<span class="text-gray-400">({stat.wordRange})</span>
					</div>
					<div class="flex flex-shrink-0 items-center gap-2">
						<span class="font-mono text-gray-500">{stat.count}</span>
						<span class="text-gray-400">({stat.percentage.toFixed(0)}%)</span>
					</div>
				</div>
				<div
					class="h-1.5 overflow-hidden rounded-full"
					style="background-color: {stat.category === 'stub' &&
					[
						'#ebedf0',
						'#f1f5f9',
						'#fef3c7',
						...
						'#fef2f2',
						'#fefce8'
					].includes(stat.color.toLowerCase())
						? '#222'
						: '#e5e7eb'}"
				>
					<div
	...
	<div class="mt-4 border-t border-gray-200 pt-3">
		<details class="group">
			<summary
				class="mb-2 cursor-pointer text-xs text-gray-500 transition-colors hover:text-gray-700"
			>
				<span class="inline-flex items-center gap-1">
					<i class="fas fa-layer-group transition-transform group-open:rotate-180"></i>
					Detailed breakdown
				</span>
			</summary>
							...
							<div class="rounded border border-gray-200 bg-white p-2">
								<div class="mb-1 flex items-center gap-1">
									...
									<span class="text-xs font-medium text-gray-700 capitalize"
										>{stat.displayName}</span
									>
								</div>
								<div class="font-mono text-xs text-gray-500">{wordRanges[stat.category]} words</div>
								<div class="font-semibold text-gray-700">{stat.count} pages</div>
							</div>
						</A>
					{/each}
				</div>
	...
	<div class="mt-3 border-t border-gray-200 pt-3">
		<details class="group">
			<summary class="cursor-pointer text-xs text-gray-500 transition-colors hover:text-gray-700">
				<span class="inline-flex items-center gap-1">
					<i class="fas fa-info-circle transition-transform group-open:rotate-180"></i>
					Word count ranges
				</span>
			</summary>
			<div class="mt-2 space-y-1 pl-4 text-xs text-gray-500">
	...
	<div class="mt-3 border-t border-gray-200 pt-3">
		<details class="group">
			<summary class="cursor-pointer text-xs text-gray-500 transition-colors hover:text-gray-700">
				<span class="inline-flex items-center gap-1">
					<i class="fas fa-palette transition-transform group-open:rotate-180"></i>
					Color scheme
				</span>
			</summary>
