<script lang="ts">
	...
	let moduleCache = $state(
		new Map<string, { moduleData: any; cleanContent: string; title: string }>()
	);
	...
	function performSearch(query: string) {
		...
		const searchResultsData = results
			.map((id) => {
			...
			.filter(
				(result): result is { id: string; title: string; path: string; excerpt: string } =>
					result !== null
			);
	...
	setContext('showSearchModal', {
		get: () => showSearchModal,
		set: (v: boolean) => (showSearchModal = v)
	});
...
<nav class="summary-nav max-h-[calc(100vh-8rem)] overflow-y-auto pb-8">
	...
	<div class="mb-6 p-2">
		<button
			...
			class="flex w-full cursor-text items-center gap-3 rounded-lg border border-gray-200 bg-white px-4 py-3 text-left transition-colors duration-200 hover:border-gray-300"
		>
			<i class="fas fa-search text-sm text-gray-400"></i>
			<span class="flex-1 text-sm text-gray-500">Search documentation...</span>
			<kbd
				class="hidden items-center rounded border border-gray-200 bg-gray-100 px-2 py-1 text-xs text-gray-500 sm:inline-flex"
			>
...
{#if showSearchModal}
	...
	<div
		class="fixed top-20 left-1/2 z-50 mx-4 w-full max-w-2xl -translate-x-1/2 transform"
		bind:this={searchModalRef}
	>
			...
			<div class="max-h-96 overflow-y-auto">
					...
					<div class="p-2">
						{#each $searchResults as result, i}
							<a
								...
								class="search-result block border-b border-gray-100 p-4 transition-colors last:border-b-0 hover:bg-gray-50 {i ===
								selectedIndex
							...
							>
								<div class="flex items-start gap-3">
									<i class="fas fa-file-text mt-1 text-sm text-blue-500"></i>
				...
				{:else if searchQuery.trim() && $searchResults.length === 0}
					<div class="p-8 text-center">
						<i class="fas fa-search mb-3 block text-2xl text-gray-300"></i>
						<p class="mb-1 text-gray-500">No results found for "{searchQuery}"</p>
						<p class="text-sm text-gray-400">Try different keywords</p>
					</div>
				{:else}
					<div class="p-8 text-center">
						<i class="fas fa-search mb-3 block text-2xl text-gray-300"></i>
						<p class="mb-1 text-gray-500">Start typing to search</p>
						<p class="text-sm text-gray-400">
							Press <kbd class="rounded bg-gray-100 px-2 py-1 text-xs">Enter</kbd> to select first result
						</p>
					</div>
				{/if}
			</div>
