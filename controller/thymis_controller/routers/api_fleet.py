...
@router.get("/fleet/connectivity", response_model=list[models.ConnectivityPoint])
...
@router.get("/fleet/metrics/latest", response_model=list[models.FleetDeviceMetric])
def get_latest_fleet_metrics(db_session: DBSessionAD):
    return crud_device_metric.get_latest_per_device(db_session)


@router.get("/fleet/availability", response_model=models.FleetAvailability)
...
@router.get("/fleet/alerts", response_model=list[models.FleetAlert])
