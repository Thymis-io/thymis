...
async def callback(code: str, response: Response, db_session: DBSessionAD):
    ...
    print("JWT Token: ", json.dumps(jwt_token, indent=4))

    if global_settings.AUTH_OAUTH_CLIENT_ROLE_LOGIN:
        client_roles = jwt_token.get("resource_access").get(global_settings.AUTH_OAUTH_CLIENT_ID)
        if not client_roles or global_settings.AUTH_OAUTH_CLIENT_ROLE_LOGIN not in client_roles.get(
            "roles", []
        ):
            raise HTTPException(
