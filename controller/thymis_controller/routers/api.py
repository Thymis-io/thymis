...
async def deploy(
    ...
    repo.commit(summary, request.state.notification_manager)
...
async def build_download_image(
    ...
    await project.create_build_device_image_task(
        identifier, db_session, request.state.notification_manager
    )
...
async def device_and_build_download_image_for_clone(
    ...
    await project.create_build_device_image_task(
        new_identifier, db_session, request.state.notification_manager
    )
...
def get_history(request: Request, repo: project.Repo = Depends(get_repo)):
    return repo.history(request.state.notification_manager)
...
def update_git_remote(
    ...
    repo.update_remote(remote, remote_update, request.state.notification_manager)
...
def switch_remote_branch(
    ...
    repo.switch_remote_branch(branch, request.state.notification_manager)
