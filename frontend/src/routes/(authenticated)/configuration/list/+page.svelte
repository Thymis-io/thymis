...
<Page title={$t('configurations.title')} subtitle={$t('configurations.subtitle')}>
	...
	<DataTable
		columns={[
			{ class: 'w-12' },
			{ label: $t('configurations.table.name') },
			{ label: $t('configurations.table.tags') },
			{ label: $t('configurations.table.devices') },
			{ label: $t('configurations.table.actions'), align: 'right' }
		]}
		rows={configs}
	...
	>
		{#snippet row(config)}
			...
			<td>
				<RowActions>
					<RowMenu
						label={$t('configurations.table.actions')}
						items={[
							{
